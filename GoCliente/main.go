...
func deleteProductoID(idProd string, canalRespuestas chan *http.Response) {
	cliente := http.Client{}
	url := "http://127.0.0.1:8000/tienda/" + idProd
	req, err := http.NewRequest("DELETE", url, nil)
	if err != nil {
		log.Fatal("Error conmiendo servicio tiendas: ", idProd)
		os.Exit(1)
	}
	resp, err := cliente.Do(req)
	if err != nil {
...
func main() {
	...
	case 3:
		...
		tiendaStr := model.Tienda{codigoTienda, nombreTienda, telefonoTienda, ciudadTienda, direccionTienda, descripcionTienda}
		...
		prodStr := model.Producto{codigoProducto, nombreProducto, precioProducto, descripcionProducto, tiendaProducto}
		var resProducto, resTienda *http.Response
